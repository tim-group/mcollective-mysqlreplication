...
describe MCollective::Agent::Mysqlreplication, :mcollective => true do
  ...
  def expect_mysql_to_run_mock_output(options)
    stdout = mock
    stdout.expects(:read).returns(options[:stdout])

    status = mock
    status.expects(:exitstatus).returns(options[:exit_code])

    stderr = options[:stderr]
    Open4.expects(:popen4).with('mysql', '-e', options[:expected_command]).returns([100, '', stdout, stderr])
  ...
  it 'responds with an error if there is no config file location fact available' do
    ...
    expect_mysql_to_run_mock_output (
      {
        :expected_command => 'show slave status \\G',
        :stdout => dummy_output,
        :sterr => '',
        :exit_code => 0
      }

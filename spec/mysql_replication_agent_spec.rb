...
describe MCollective::Agent::Mysqlreplication, :mcollective => true do
  ...
  before(:each) do
    ...
    @successful_status = mock
    @failed_status = mock
    #@failed_status.expects(:exitstatus).returns(0)
  end
  ...
  it 'responds with an error if there is no config file location fact available' do
    ...
    @successful_status.expects(:exitstatus).returns(0)
    @dummy = mock
    @dummy.expects(:read).returns(dummy_output)
    Open4.expects(:popen4).with('mysql', '-e', 'show slave status \\G').returns([1, '', @dummy, ''])
    Process.expects(:waitpid2).with(1).returns([nil, @successful_status])
